...
export interface Props {
  message: Message;
  messageIndex: number;
  // appendStreamedContent: (chunk: string) => void; // New prop
}
...
export const ChatMessage: FC<Props> = memo(({ message, messageIndex }) => {
  ...
  const {
    state: { selectedConversation, settings },
  ...
  return (
    <div
      className={`group md:px-4 ${message.role === 'assistant'
        ? 'border-b border-black/10 bg-gray-50 text-gray-800 dark:border-gray-900/50 dark:bg-color_light dark:text-gray-100'
        : 'border-b border-black/10 bg-white text-gray-800 dark:border-gray-900/50 dark:bg-color_dark dark:text-gray-100'
        }`}
      ...
      <div className="relative m-auto flex p-4 text-base md:max-w-2xl md:gap-6 md:py-6 lg:max-w-2xl lg:px-0 xl:max-w-3xl">
        <div className="min-w-[40px] text-right font-bold">
          {message.role === 'assistant' ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'left' }}>
              <Avatar username={settings.displayName} size={25} />
          ...
          {message.role === 'user' ? (
            <div className="flex w-full">
              {isEditing ? (
                ...
                <div className="prose whitespace-pre-wrap dark:prose-invert flex-1 flex items-start text-left">
