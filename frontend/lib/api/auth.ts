...
export const apiAuth = {
  ...
  async loginWithOauth(username: string, password: string): Promise<ITokenResponse> {
    ...
    const res = await fetch(`${apiCore.url}/login/oauth`, {
      method: "POST",
      body: params,
      headers: { "Content-Type": "application/x-www-form-urlencoded" }, // Corrected header
    });
  ...
  async authenticate(username: string, password: string): Promise<IUserProfile> {
    const params = new URLSearchParams();
    params.append("username", username);
    params.append("password", password);
  
    console.log("me inside" + `${apiCore.url}`);
